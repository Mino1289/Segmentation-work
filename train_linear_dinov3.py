...
from models.dino_feature_extractor import DinoLinearADE20K, DinoV3FeatureExtractor
...
if __name__ == "__main__":
    ...
    weight_path = os.path.join(weight_dir, "linear_dinov3_base.pth")
    best_mIoU = 0.0

    device = get_device()
    print(f"Using device: {device}")

    config = DinoV3Config("B")  # embed_dim = 768
    dino = DinoV3(config=config, pretrained=True)
