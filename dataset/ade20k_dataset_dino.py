...
def get_dataloaders(batch_size=4, num_workers=0, resolution=518):
    ...
    train_transforms = v2.Compose([
        v2.RandomResize(min_size=int(resolution * SCALE_RANGE[0]), max_size=int(resolution * SCALE_RANGE[1]), antialias=True),
        v2.RandomCrop(size=(resolution, resolution), pad_if_needed=True, fill={tv_tensors.Image: 0, tv_tensors.Mask: 255}),
    ...
    val_dataset = ADE20KDatasetDino(raw_dataset["validation"], transform=val_test_transforms)
...
if __name__ == "__main__":
    ...
    for images, masks in train_loader:
        print(f" -> Images (Train) shape : {images.shape} (Attendu: [{BATCH_SIZE}, 3, {RESOLUTION}, {RESOLUTION}])")
        print(f" -> Masks  (Train) shape : {masks.shape} (Attendu: [{BATCH_SIZE}, {RESOLUTION}, {RESOLUTION}])")
        print(f" -> Masks  Dtype         : {masks.dtype} (Attendu: torch.int64)")
        break

    print("\nVérification d'un batch du Test Loader...")
    for images, masks in test_loader:
        print(f" -> Images (Test) shape  : {images.shape} (Attendu: [1, 3, {RESOLUTION}, {RESOLUTION}])")
        print(f" -> Masks  (Test) shape  : {masks.shape} (Attendu: [1, {RESOLUTION}, {RESOLUTION}])")
        break