...
DEFAULT_DATASETS = {
    "DINOv2-ViT-Base": Path(__file__).with_name("metrics-dinov2-base.csv"),
    "DINOv3-ViT-Base": Path(__file__).with_name("metrics-dinov3-base.csv"),
    "DINOv3-ViT-Large": Path(__file__).with_name("metrics-dinov3-large.csv"),
    "UPerNet-CNX-Base": Path(__file__).with_name("metrics-upernet.csv"),
}
...
def collect_metric_names(
    series: dict[str, list[dict[str, float | int | None]]],
) -> list[str]:
    metric_names: set[str] = set()
    for rows in series.values():
        for row in rows:
            metric_names.update(
                key for key in row.keys() if key != "epoch" and row.get(key) is not None
            )
...
def plot_metrics(
    series: dict[str, list[dict[str, float | int | None]]], output: Path | None
) -> None:
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Plot metrics from the three training CSV files."
    )
    parser.add_argument(
        "--dinov2",
        type=Path,
        default=DEFAULT_DATASETS["DINOv2-ViT-Base"],
        help="Path to metrics-dinov2.csv",
    )
    parser.add_argument(
        "--dinov3",
        type=Path,
        default=DEFAULT_DATASETS["DINOv3-ViT-Base"],
        help="Path to metrics-dinov3.csv",
    )
    parser.add_argument(
        "--dinov3-large",
        type=Path,
        default=DEFAULT_DATASETS["DINOv3-ViT-Large"],
        help="Path to metrics-dinov3-large.csv",
    )
    parser.add_argument(
        "--upernet",
        type=Path,
        default=DEFAULT_DATASETS["UPerNet-CNX-Base"],
...
def main() -> None:
    ...
    series = {
        "DINOv2-ViT-Base": load_metrics(args.dinov2),
        "DINOv3-ViT-Base": load_metrics(args.dinov3),
        "DINOv3-ViT-Large": load_metrics(args.dinov3_large),
        "UPerNet-CNX-Base": load_metrics(args.upernet),
    }
